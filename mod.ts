...
const config: any = {
	util: {
		async loadConfig(configOptions?: ConfigOptions): Promise<void> {

			const baseDir = configOptions?.baseDir || resolve(Deno.cwd(),'config');
	
			const configPathReadDesc = { name: "read", path: baseDir } as const;
			...
			for (const nodeEnvVarName of nodeEnvVarNames) {
				...
				if (configOptions?.env && configOptions?.env[nodeEnvVarName]) {
					nodeEnv = configOptions?.env[nodeEnvVarName];
					break;
				}
			...
			for (const allowedFile of allowedFiles) {
				...
				const filePath = resolve(baseDir, allowedFile.fileName);
			...
			if (configOptions?.env) {
				...
				for (const key in configOptions?.env) {
					env[key] = configOptions?.env[key];
				}
			}
				...
				for (const allowedFile of allowedFiles) {
                    ...
                    try {
						...
						if (configObj) {
							...
							extendDeep(config, environmentSubstitutions);
						}
...
Object.defineProperty(config, "util", {
    enumerable : false
});

await config.util.loadConfig();

export async function loadConfig(configOptions?: ConfigOptions) {
    for (const key in config) {
        delete config[key];
    }
    await config.util.loadConfig(configOptions);
}
